...
PTR seglist;
	...
	MYDEBUG( "SANA2 : _manager_Close : IOReq %p", ioreq );
	...
	if ( ! unit )
	{
		MYERROR( "SANA2 : Unit NULL Pointer" );
		goto bailout;
	}
	...
	if ( unit->unit_Unit.unit_OpenCnt )
	{
		...
		MYERROR( "SANA2 : Unit open count not Zero : OpenCnt %ld", unit->unit_Unit.unit_OpenCnt );
		goto bailout;
	}

	// Clear Open Array Ptr
	devBase->dev_SANA2_Units[ unit->unit_UnitNr ] = NULL;
