...
struct Device *					USBSysBase			= NULL;		// Controlled from USB.c
...
struct USBSysIFace *			IUSBSys				= NULL;		// Controlled from USB.c
...
//	{ OT_Library,		"layers.library",			50,		& LayersBase,		NULL },
	{ OT_Library,		"usbresource.library",		1,		& USBResourceBase,	NULL },
...
//	{ OT_Interface,		"main",						1,		& LayersBase,		& ILayers },
	{ OT_Interface,		"main",						1,		& USBResourceBase,	& IUSBResource },
	...
	for( pos=0 ; pos < max_libs ; pos++ )
	{
		...
		switch( OpenList[pos].os_Type )
		{
			case OT_Library:
			{
				...
				if ( ! base )
				{
					MYERROR( "USBFD : Error missing base pointer" );
					goto bailout;
				}
				...
				if ( *base == NULL )
				{
					...
					MYINFO( "USBFD : Opening library '%s' v%lu", name, vers );

					ptr = OpenLibrary( name, vers );

					if ( ! ptr )
					{
						MYERROR( "USBFD : Error opening library '%s' v%lu", name, vers );
						goto bailout;
					}
			...
			case OT_Interface:
			{
				...
				if ( ! ifc )
				{
					MYERROR( "USBFD : Error missing interface pointer" );
					goto bailout;
				}
				...
				if ( *ifc == NULL )
				{
					...
					MYINFO( "USBFD : Getting interface pos #%lu", pos );

					ptr = GetInterface( *base, name, vers, NULL );

					if ( ! ptr )
					{
						MYERROR( "USBFD : Error getting interface pos #%lu", pos );
						goto bailout;
					}
			...
			case OT_Class:
			{
				...
				if ( ! base )
				{
					MYERROR( "USBFD : Error missing class pointer" );
					goto bailout;
				}
				...
				if ( *base == NULL )
				{
					...
					if ( ! IIntuition )
					{
						MYERROR( "USBFD : Error need Intuition" );
						goto bailout;
					}
					...
					MYINFO( "USBFD : Opening class '%s'", name );

					ptr = OpenClass( name, vers, cls );

					if ( ! ptr )
					{
						MYERROR( "USBFD : Error opening class '%s' pos #%lu", name, pos );
						goto bailout;
					}
			...
			default:
			{
				MYERROR( "USBFD : Unknown Resource type (%lu)", OpenList[pos].os_Type );
				goto bailout;
			}
		}
	}
...
S32 pos;
	...
	MYERROR( "USBFD : myCloseResources" );
	...
	{
		pos--;
		MYERROR( "USBFD : Processing : %s", OpenList[pos].os_STR );
