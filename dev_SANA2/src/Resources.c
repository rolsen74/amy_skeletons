...
struct Device *					TimerBase			= NULL;		// Handled else where
...
//	{ OT_Interface,		"main",						1,		& P96Base,			& IP96 },
	...
	{ OT_Interface,		"main",						1,		& TimerBase,		& ITimer },
	...
	for( pos=0 ; pos < max_libs ; pos++ )
	{
		...
		switch( OpenList[pos].os_Type )
		{
			case OT_Library:
			{
				...
				if ( ! base )
				{
					MYERROR( "SANA2 : Error missing base pointer" );
					goto bailout;
				}
				...
				if ( *base == NULL )
				{
					...
					MYINFO( "SANA2 : Opening library '%s' v%lu", name, vers );

					ptr = OpenLibrary( name, vers );

					if ( ! ptr )
					{
						MYERROR( "SANA2 : Error opening library '%s' v%lu", name, vers );
						goto bailout;
					}
			...
			case OT_Interface:
			{
				...
				if ( ! ifc )
				{
					MYERROR( "SANA2 : Error missing interface pointer" );
					goto bailout;
				}
				...
				if ( *ifc == NULL )
				{
					...
					MYINFO( "SANA2 : Getting interface pos #%lu", pos );

					ptr = GetInterface( *base, name, vers, NULL );

					if ( ! ptr )
					{
						MYERROR( "SANA2 : Error getting interface pos #%lu", pos );
						goto bailout;
					}
			...
			case OT_Class:
			{
				...
				if ( ! base )
				{
					MYERROR( "SANA2 : Error missing class pointer" );
					goto bailout;
				}
				...
				if ( *base == NULL )
				{
					...
					if ( ! IIntuition )
					{
						MYERROR( "SANA2 : Error need Intuition" );
						goto bailout;
					}
					...
					MYINFO( "SANA2 : Opening class '%s'", name );

					ptr = OpenClass( name, vers, cls );

					if ( ! ptr )
					{
						MYERROR( "SANA2 : Error opening class '%s' pos #%lu", name, pos );
						goto bailout;
					}
			...
			default:
			{
				MYERROR( "SANA2 : Unknown Resource type (%lu)", OpenList[pos].os_Type );
				goto bailout;
			}
		}
	}
...
S32 pos;
	...
	MYERROR( "SANA2 : myCloseResources" );
	...
	{
		pos--;
		MYERROR( "SANA2 : Processing : %s", OpenList[pos].os_STR );
