...
S32 sig;
	...
	if ( ! unit )
	{
		MYERROR( "SANA2 : Unit_Startup : NULL Pointer" );
		goto bailout;
	}

	MYDEBUG( "SANA2 : Unit_Startup : UnitNr #%lu", unit->unit_UnitNr );
	...
	if ( ! msg.Result )
	{
		MYERROR( "SANA2 : Failed to SANA2 process" );
		goto bailout;
	}
...
bailout:
	...
	MYDEBUG( "SANA2 : Unit_Startup : Leave" );
